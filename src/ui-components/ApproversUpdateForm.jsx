...
import { getApprovers } from "../graphql/queries";
import { updateApprovers } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function ApproversUpdateForm(props) {
  const {
    id: idProp,
    approvers: approversModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = approversRecord
      ? { ...initialValues, ...approversRecord }
      : initialValues;
    setName(cleanValues.name);
    setType(cleanValues.type);
    setApprovers(cleanValues.approvers ?? []);
  ...
  const [approversRecord, setApproversRecord] =
    React.useState(approversModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getApprovers.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getApprovers
        : approversModelProp;
      setApproversRecord(record);
    };
    queryData();
  }, [idProp, approversModelProp]);
  React.useEffect(resetStateValues, [approversRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateApprovers.replaceAll("__typename", ""),
            variables: {
              input: {
                id: approversRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ApproversUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || approversModelProp)}
