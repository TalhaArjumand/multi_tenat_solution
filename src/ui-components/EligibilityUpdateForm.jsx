...
import { getEligibility } from "../graphql/queries";
import { updateEligibility } from "../graphql/mutations";
export default function EligibilityUpdateForm(props) {
  const {
    id: idProp,
    eligibility: eligibilityModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = eligibilityRecord
      ? { ...initialValues, ...eligibilityRecord }
      : initialValues;
    setName(cleanValues.name);
    setType(cleanValues.type);
    setTicketNo(cleanValues.ticketNo);
    setApprovalRequired(cleanValues.approvalRequired);
  ...
  const [eligibilityRecord, setEligibilityRecord] =
    React.useState(eligibilityModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getEligibility.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getEligibility
        : eligibilityModelProp;
      setEligibilityRecord(record);
    };
    queryData();
  }, [idProp, eligibilityModelProp]);
  React.useEffect(resetStateValues, [eligibilityRecord]);
  const validations = {
    name: [],
    type: [],
    ticketNo: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateEligibility.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "EligibilityUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || eligibilityModelProp)}
